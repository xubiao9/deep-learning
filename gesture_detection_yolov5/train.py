...
if __name__ == "__main__":
    ...
    # model_path = 'logs/loss_2023_11_06_01_00_27/last_epoch_weights.pth'
    # model_path = ''
    #------------------------------------------------------#
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    #   训练分为两个阶段，分别是  冻结阶段  和  解冻阶段  。设置冻结阶段是为了满足机器性能不足的同学的训练需求。
    ...
    #------------------------------------------------------------------#
    UnFreeze_Epoch      = 300
    Unfreeze_batch_size = 2 # 原来是8
    # 从 0 开始训练，冻结主干训练到 50 代，后解冻训练回归框，到 300 代
    ...
    #------------------------------------------------------------------#
    optimizer_type      = "sgd"
    momentum            = 0.937  # 动量，梯度下降直接计算，momentum还计算前面梯度的加权平均，用于权值优化
    weight_decay        = 5e-4  # lr正则化的权值衰减参数
    #------------------------------------------------------------------#
    #   lr_decay_type   使用到的学习率下降方式，可选的有step、cos
    #------------------------------------------------------------------#
    lr_decay_type       = "cos" # 余弦退火，# cos：先上升后下降、 step：直接下降
    #------------------------------------------------------------------#
    ...
    #------------------------------------------------------#
    train_annotation_path   = 'gesture_train.txt'
    val_annotation_path     = 'gesture_val.txt'

    seed_everything(seed) # 11
    #------------------------------------------------------#
    #   设置用到的显卡
    #------------------------------------------------------#
    ngpus_per_node  = torch.cuda.device_count() # 1，因为就一个 gpu
    ...
    if model_path != '':
        ...
        #------------------------------------------------------#
        if local_rank == 0:
            print('Load weights {}.'.format(model_path)) # 加载权重
        ...
        for k, v in pretrained_dict.items():
            if k in model_dict.keys() and np.shape(model_dict[k]) == np.shape(v):
                temp_dict[k] = v # 提取预训练权重中与训练所用数据集匹配的 key 和 value
                load_key.append(k)
            else:
                no_load_key.append(k)

        model_dict.update(temp_dict) # 用提取的权重对参数进行更新
        model.load_state_dict(model_dict)  # 加载到模型中
    ...
    if True:
        ...
        #---------------------------------------#
        pg0, pg1, pg2 = [], [], []  
        for k, v in model.named_modules(): # 返回模型中包含的所有模块对象及其子模块对象，并且会为每个模块对象分配一个唯一的名称
            if hasattr(v, "bias") and isinstance(v.bias, nn.Parameter):
                # isinstance判断类型，hasattr判断对象是否包含对应的属性
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            set_optimizer_lr(optimizer, lr_scheduler_func, epoch) # 学习率更新
