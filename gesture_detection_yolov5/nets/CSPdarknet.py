...
class C3(nn.Module):
    ...
    def forward(self, x):
        return self.cv3(torch.cat(  # b, c, h, w # CSP
            (
                self.m(self.cv1(x)), # 输出c_
                self.cv2(x)  # 输出c_
            )
            , dim=1))
...
class CSPDarknet(nn.Module):
    def __init__(self, base_channels, base_depth, phi, pretrained):
        ...
        # -----------------------------------------------#
        #   利用focus网络结构进行特征提取，不是卷积变的320，Focus就会降低尺寸
        #   640, 640, 3 -> 320, 320, 3*4=12 -> 320, 320, 64
        # -----------------------------------------------#
    ...
    def forward(self, x):
        ...
        x = self.dark5(x)
        feat3 = x
        return feat1, feat2, feat3 # 3个特征层
