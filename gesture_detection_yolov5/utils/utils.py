...
def get_classes(classes_path):
    ...
    class_names = [c.strip() for c in class_names] # strip方法用于移除字符串首尾指定的字符串；当没有参数时，默认为空格和换行符。
    return class_names, len(class_names) # [len(class_names)]个元素的列表
...
def get_anchors(anchors_path):
    ...
    with open(anchors_path, encoding='utf-8') as f:
        anchors = f.readline()
    anchors = [float(x) for x in anchors.split(',')] # 去除所有值
    anchors = np.array(anchors).reshape(-1, 2)       # [9, 2]
