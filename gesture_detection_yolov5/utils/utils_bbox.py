...
class DecodeBox():
    ...
    def decode_box(self, inputs):
        ...
        for i, input in enumerate(inputs):
            ...
            #   batch_size = 1
            #   batch_size, 3 * (4 + 1 + 80), 20, 20 # 这里是coco数据集，有80个类
            ...
            # 将预测框除上缩放系数，就获得预测框归一化后的结果了
            # conf 是置信度
            # pre_cls 种类置信度
            # -1表示在最后一个维度拼接
            # print(_scale) : tensor([640., 640., 640., 640.])
    ...
    def yolo_correct_boxes(self, box_xy, box_wh, input_shape, image_shape, letterbox_image):
        ...
        box_yx = box_xy[..., ::-1] # 倒序
        box_hw = box_wh[..., ::-1]
        input_shape = np.array(input_shape) # 640 * 640
        image_shape = np.array(image_shape) # e.g.  500 * 600
        # 640 / 500 = 1.28    640 / 600 = 1.07
        # 1.07 * 500 = 530    1.07 * 600 = 640
        ...
        if letterbox_image:
            ...
            #   new_shape指的是宽高缩放情况
            #-----------------------------------------------------------------#
            # np.round()函数的做用：对给定的数组进行四舍五入，能够指定精度
    ...
    # input_shape = (640, 640)  image_shape:resize后的
    def non_max_suppression(self, prediction, num_classes, input_shape, image_shape, letterbox_image, conf_thres=0.5, nms_thres=0.4):
        #----------------------------------------------------------#
        #   将预测结果的格式从 中心宽高 转换成 左上角右下角 的格式。
        ...
        for i, image_pred in enumerate(prediction):
            # 每次取一个batch
            # 所以这里的 image_pred 是 [num_anchors, 85]
            #----------------------------------------------------------#
            #   对种类预测部分取max。
            #   class_conf  [num_anchors, 1]    种类置信度（返回最大值）
            #   class_pred  [num_anchors, 1]    种类，属于哪个种类（返回最大值的索引）
            #----------------------------------------------------------#
            class_conf, class_pred = torch.max(image_pred[:, 5:5 + num_classes], 1, keepdim=True)
            # 1：寻找每一行的最大值，0：寻找每一列的最大值
            ...
            #   根据置信度进行预测结果的筛选
            #----------------------------------------------------------#
            # image_pred : [batch_size, num_anchors, 85]
            # 如，conf_mask= tensor([False, False,  True])
            # 就是返回 True 的内容，False 就不返回
            # 保留预测框
            image_pred = image_pred[conf_mask]
            class_conf = class_conf[conf_mask]
            class_pred = class_pred[conf_mask]
            if not image_pred.size(0): # 表示没有框满足置信度要求，就跳过后面的处理，也就是说这张图片没有预测到任何内容
                continue
            #-------------------------------------------------------------------------#
            ...
            #   获得预测结果中包含的所有种类
            #------------------------------------------#
            unique_labels = detections[:, -1].cpu().unique() # 取所有行的最后一列，就是 class_pred
            ...
            for c in unique_labels:
                ...
                #   找每一个框的筛选出来的类的 nms
                keep = nms(
                    detections_class[:, :4], # 取筛选出来的类的 左上和右下点
                    detections_class[:, 4] * detections_class[:, 5], # 取 obj_conf * class_conf
                    nms_thres
                )
                max_detections = detections_class[keep] # 找到3个框里，最大概率的
...
if __name__ == "__main__":
    ...
    #   将预测值的每个特征层调成真实值
    #---------------------------------------------------#
    # input 是 yolo-v5 网络的输出（3个特征层）
    # 这里以 20 * 20 特征层为输入例子
    def get_anchors_and_decode(input, input_shape, anchors, anchors_mask, num_classes):
        #-----------------------------------------------#
        #   input   batch_size, 3 * (4 + 1 + num_classes), 20, 20 # <- head层有个1x1卷积，将1024变为3*(5+num_classes)
        #   3 是每个特征点的3个先验框，3个特征图，每个3个框，共9个
        #   4 是先验框的回归系数，1 代表先验框内部是否包含物体，num_classes代表先验框内部包含物体的种类
        #-----------------------------------------------#
        batch_size      = input.size(0)
        input_height    = input.size(2)
        input_width     = input.size(3)
        ...
        #   [batch_size, 3, 20, 20]
        #----------------------------------------------------------#
        # 首先生成 0 - 19 的列表，20（input_width）个点，在高上重复20次，再映射到每个batch和先验框上
        # 所以再重复 batch_szie * 先验框个数 次
        # 每个先验框的x轴坐标
        # .repeat()是在某个维度上重复几次
        # --------------- 相当于创建特征点的坐标 ------------------
        grid_x = torch.linspace(0, input_width - 1, input_width).repeat(input_height, 1).repeat(
        ...
        #   batch_size, 3, 20 * 20 => batch_size, 3, 20, 20
        #----------------------------------------------------------#
        # index_select 从第1维开始，挑选LongTensor([0])位置的数据
        # scaled_anchors 是 （3， width，height） 3是3个框 维度(3, 1, 2)
        # -------------------- 相当于每个特征点上的先验框 -------------------
        anchor_w = FloatTensor(scaled_anchors).index_select(1, LongTensor([0]))
        anchor_h = FloatTensor(scaled_anchors).index_select(1, LongTensor([1]))
        anchor_w = anchor_w.repeat(batch_size, 1).repeat(1, 1, input_height * input_width).view(w.shape)
        ...
        plt.imshow(img, alpha=0.5)  # alpha 是颜色强度
        plt.ylim(-30, 650)
        plt.xlim(-30, 650)
