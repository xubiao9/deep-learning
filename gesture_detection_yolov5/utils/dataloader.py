...
class YoloDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # box : txt 里的 box 值
        # y_true : [3个特征层, 每个特征层3个框, x, y, w, h, 是否是物体, 80个种类（真实框种类=1）]
    ...
    def get_random_data(self, annotation_line, input_shape, jitter=.3, hue=.1, sat=0.7, val=0.4, random=True):
        ...
        #   读取图像并转换成RGB图像
        #------------------------------#
        image   = Image.open(line[0]) # 图片路径
        image   = cvtColor(image) # 非RGB转化成RGB打开，cv2颜色是BGR的
        #------------------------------#
        #   获得图像的高宽与目标高宽
        #------------------------------#
        iw, ih  = image.size # 图片的w, h
    ...
    def get_target(self, targets):
        ...
        for l in range(num_layers):
            ...
            # x, y, w, h 中心点xy
            # 把 box 转换到特征层上的尺寸
            batch_target[:, [0,2]]  = targets[:, [0,2]] * in_w # 预测出的框是特征图的，要转化到原尺寸上
            ...
            for t, ratio in enumerate(max_ratios):
                ...
                for k, mask in enumerate(self.anchors_mask[l]):
                    ...
                    for offset in offsets:
                        ...
                        y_true[l][k, local_j, local_i, 4] = 1      # 是否有物体
                        y_true[l][k, local_j, local_i, c + 5] = 1  # 真实框种类
                        #----------------------------------------#
                        #   获得当前先验框最好的比例
                        #----------------------------------------#
