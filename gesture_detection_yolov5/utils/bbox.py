...
class DecodeBox():
    ...
    def decode_box(self, inputs):
        ...
        for i, input in enumerate(inputs):
            ...
            #   将输出结果归一化成小数的形式
            # ----------------------------------------------------------#
            # pred_boxes: [batch_size,3,20,20,4]
            _scale = torch.Tensor([input_width, input_height, input_width, input_height]).type(FloatTensor)
            output = torch.cat((pred_boxes.view(batch_size, -1, 4) / _scale,
                                conf.view(batch_size, -1, 1), pred_cls.view(batch_size, -1, self.num_classes)), -1)
            # -1 对最后一个维度拼接
            # output: [batch_size,3,20,20,4+1+num_class]

            outputs.append(output.data)
            # outputs: [3,batch_size,3,20,20,4+1+num_class] # 有3个输入
        return outputs
    ...
    def yolo_correct_boxes(self, box_xy, box_wh, input_shape, image_shape, letterbox_image):
        ...
        if letterbox_image:
            ...
            #   new_shape指的是宽高缩放情况
            # -----------------------------------------------------------------#
            new_shape = np.round(image_shape * np.min(input_shape / image_shape)) # 640, 640
            offset = (input_shape - new_shape) / 2. / input_shape # 0
            scale = input_shape / new_shape # 1
        ...
        # np.concatenate([image_shape, image_shape], axis=-1) 后是(640, 640, 640, 640)
        # 每列对应相乘，获得原图上的中心点 [batch, xywh]
        # [num_same_c, 4]
        return boxes

    def non_max_suppression(self, prediction, num_classes, input_shape, image_shape, letterbox_image, conf_thres=0.5,
                            nms_thres=0.4):
        # prediction：([4, 3, 20, 20, 4]) -> torch.Size([4, 1200, 85]) 4是batch
        # ----------------------------------------------------------#
        ...
        output = [None for _ in range(len(prediction))] # len(prediction) = 4
        # output用 None 占位，[None, None, None, None]
        # [4, 1200, 85]
        # [1200, 85] = [3*20*20, 85]
        for i, image_pred in enumerate(prediction):  # [3*20*20, 85]
            # ----------------------------------------------------------#
            #   对种类预测部分取max。
            ...
            # class_conf是值
            # class_pred是索引
            # keepdim=True：保持原数组的维度，[3*20*20, 1]

            # ----------------------------------------------------------#
            #   利用置信度进行第一轮筛选
            # ----------------------------------------------------------#
            conf_mask = (image_pred[:, 4] * class_conf[:, 0] >= conf_thres).squeeze() # squeeze()去除1维度
            # [3*20*20], 满足的为 True，否则为False
            ...
            # ------------------------------------------#
            # [num], num为满足阈值的
            unique_labels = detections[:, -1].cpu().unique() # unique()去除重复元素并从小到大排序

            if prediction.is_cuda:
                unique_labels = unique_labels.cuda()
                detections = detections.cuda()

            for c in unique_labels: # 每个点每个先验框的分类
                # ------------------------------------------#
                #   获得某一类得分筛选后全部的预测结果
                # ------------------------------------------#
                detections_class = detections[detections[:, -1] == c]
                # [num2, 7] , num2 为种类为7的个数
                ...
                # [c_num, 7]   , c_num为所有检测到的类别个数，都是nms后的

            # 把框调整成相对于原图的样式，因为上面用了不失真的 resize
            # 所以要把原点从上面灰条的左上角 转换到 resize 后的左上角
            #   detections  [3*20*20, 7]
            #       7的内容为：x1, y1, x2, y2, obj_conf, class_conf, class_pred
            # output.shape: [c_num, 4]
...
if __name__ == "__main__":
    ...
    def get_anchors_and_decode(input, input_shape, anchors, anchors_mask, num_classes):
        ...
        # -----------------------------------------------#
        stride_h = input_shape[0] / input_height  # 640 / 20 = 32
        stride_w = input_shape[1] / input_width   # 640 / 20 = 32
        # -------------------------------------------------#
        #   此时获得的scaled_anchors大小是相对于特征层的
        ...
        print(prediction.shape) # torch.Size([4, 3, 20, 20, 85])

        # -----------------------------------------------#
        #   先验框的中心位置的调整参数
        # -----------------------------------------------#
        x = torch.sigmoid(prediction[..., 0])  # 最后一个维度
        y = torch.sigmoid(prediction[..., 1])
        # -----------------------------------------------#
        #   先验框的宽高调整参数
        # -----------------------------------------------#
        ...
        #   [batch_size, 3, 20, 20]
        # ----------------------------------------------------------#
        # repeat和原维度不一样，就先添加维度
        # 生成坐标,t()转置
        # x: [batch_size, 3, 20, 20, 1]
        # 0 ~ 20, [20] -> [20, 20] -> [batch*3, 20, 20] -> [batch_size, 3, 20, 20, 1]
        # [20]: 每行 0 - 19，每列 0 - 0
        grid_x = torch.linspace(0, input_width - 1, input_width).repeat(input_height, 1).repeat(
        ...
        anchor_w = FloatTensor(scaled_anchors).index_select(1, LongTensor([0]))
        anchor_h = FloatTensor(scaled_anchors).index_select(1, LongTensor([1]))
        # [3, 1] -> bt, 3, 1 -> bt, 3, 20*20 -> [batch_size, 3, 20, 20, 1]
        # w: [batch_size, 3, 20, 20, 1]
        anchor_w = anchor_w.repeat(batch_size, 1).repeat(1, 1, input_height * input_width).view(w.shape)
        anchor_h = anchor_h.repeat(batch_size, 1).repeat(1, 1, input_height * input_width).view(h.shape)
        ...
        # ----------------------------------------------------------#
        pred_boxes = FloatTensor(prediction[..., :4].shape)
        print("pred_boxes:",pred_boxes.shape) # pred_boxes: torch.Size([4, 3, 20, 20, 4])
        pred_boxes[..., 0] = x.data * 2. - 0.5 + grid_x
        pred_boxes[..., 1] = y.data * 2. - 0.5 + grid_y
        pred_boxes[..., 2] = (w.data * 2) ** 2 * anchor_w
        pred_boxes[..., 3] = (h.data * 2) ** 2 * anchor_h
        ...
        print("output:",output.shape) # output: torch.Size([4, 1200, 85])
        # ([4, 3, 20, 20, 4]) -> torch.Size([4, 1200, 85])
        outputs = []
        outputs.append(output.data) # torch.Size([4, 1200, 85])
        # outputs = torch.stack(outputs) # stack后会多出一个维度[1, 4, 1200, 85]
        print(outputs.shape) # torch.Size([4, 1200, 85])
        ...
        fig = plt.figure()  # 新建画布（单个子图）
        ax = fig.add_subplot(121)

        from PIL import Image
        img = Image.open("../img/street.jpg").resize([640, 640])
        plt.imshow(img, alpha=0.5) # alpha是颜色的强度
        plt.ylim(-30, 650)
        plt.xlim(-30, 650)
        plt.scatter(grid_x, grid_y)
        plt.scatter(point_h * 32, point_w * 32, c='black') # 画一个黑色中心点
    ...
    # input, input_shape, anchors, anchors_mask, num_classes
