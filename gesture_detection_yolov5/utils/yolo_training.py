...
class YOLOLoss(nn.Module):
    def __init__(self, anchors, num_classes, input_shape, cuda, anchors_mask = [[6,7,8], [3,4,5], [0,1,2]], label_smoothing = 0):
        ...
        self.label_smoothing = label_smoothing # 0
    ...
    def Focal_EIou_Loss(self, box1, box2, x1y1x2y2=False, GIoU=False, DIoU=False, CIoU=False, EIoU=False, eps=1e-7):
    ...
    def Focal_EIou_Loss2(self,pbox,gbox,loss_weight=1., eps=1e-10, use_complete_iou_loss=True):
        self.use_complete_iou_loss = use_complete_iou_loss

        pbox = torch.as_tensor(pbox)
        ...
        # Focal-EIOU
        eiou = paddle.mean((1 - iouk + diou_term + eiou_term) * 1)
    ...
    #   平滑标签
    #---------------------------------------------------#
    def smooth_labels(self, y_true, label_smoothing, num_classes): # y_true
    ...
    def forward(self, l, input, targets=None, y_true=None):
        ...
        # [batch_size, 3, in_w, in_h, 4]

        if self.cuda:
            y_true          = y_true.type_as(x) # 每个特征层的真实框
        ...
        if n != 0:
            ...
            # x = x.cpu().numpy()
            focal_eiou = self.Focal_EIou_Loss(pred_boxes, y_true[..., :4]).type_as(x)
            loss_loc = torch.mean((1 - focal_eiou)[y_true[..., 4] == 1])
            ...
            #   它才是用来预测这个物体的。
            #-----------------------------------------------------------#
            tobj = torch.where(y_true[..., 4] == 1, focal_eiou.detach().clamp(0), torch.zeros_like(y_true[..., 4]))
    ...
    def get_pred_boxes(self, l, x, y, h, w, targets, scaled_anchors, in_h, in_w):
        ...
        #   计算一共有多少张图片
        #-----------------------------------------------------#
        bs = len(targets) # batch

        #-----------------------------------------------------#
        #   生成网格，先验框中心，网格左上角
        #-----------------------------------------------------#
        # in_w, in_h 为特征图的大小
        # batch*3, in_w, in_h -> batch_size, 3, in_w, in_h, 1
        ...
        # [3, 1] -> [batch*3, 2] -> [batch*3, 2, 20*20]
        # batch_size, 3, in_w, in_h, 1 ：每个图每个点3个框的w和h
